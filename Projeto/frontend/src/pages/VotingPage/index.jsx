...
const VotingPage = () => {
  ...
  if (voting.winners) {
    return (
      <div className="voting-page__winners">
        <h1>Votação Terminada! Vencedores: </h1>
        {voting.winners.map((winner) => (
  ...
  return (
    <div className="voting-page__container">
      <div className="voting-page__header">
        <h2>{voting.title}</h2>
        {voting.canVote ? (
          <button onClick={handleDoVote}>Votar</button>
        ) : (
          <p>{voting.reason}</p>
        )}
      </div>
